...
def edit_customer(customer_id):
    ...
    # Update customer
    update_data = {k: v for k, v in data.items() if k in ['name', 'phone', 'time', 'date']}
...
def get_appointments():
    ...
    if type == "history":
        appointments = list(appointments_collection.find({'status' : 'completed', 'customer_id':customer_object_id}))
    else:
        appointments = list(appointments_collection.find({'status' : 'scheduled', 'customer_id':customer_object_id}))
